...
class Message:
    ...
    def full_text(self) -> str:
        """The full text f this message, including the count if necessary"""
        if self.count > 1:
...
class MessageLog:
    ...
    def add_message(self, text: str, fg: Tuple[int, int, int] = color.white, *, stack: bool = True) -> None:
        ...
        `text` is the message text, `fg` is the text color. If `stack` is True then the message can 
    ...
    def render(self, console: tcod.Console, x: int, y: int, width: int, height: int) -> None:
        ...
        `x`, `y`, `width`, `height` is the rectangle region to render onto the console.
        """
